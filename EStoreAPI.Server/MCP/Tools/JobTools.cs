...
public class JobTools
{
    ...
    public async Task<ICollection<OutJobDTO>> CreateJobsAsync(
        [Description("Jobs to create. Each requires: CustomerId, DeviceId, and at least one ProblemId. Search the customer by name for CustomerId, search the device by name for DeviceId, then use that DeviceId to retrieve the problem catalogue and resolve ProblemIds.")] ICollection<InJobDTO> dtos)
    {
        try
        {
